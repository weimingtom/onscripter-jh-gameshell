...
#endif
#if defined(MIYOO)
if (key == 40 || key == 44) { //select==40, start==44 
	//endCommand();
        SDL_Event event;
        event.type = SDL_QUIT;
        SDL_PushEvent(&event);
//        key = SDLK_UNKNOWN;
    ...
    switch(key){
      ...
      case SDLK_LEFT:   key = SDLK_LCTRL;   break; /* vkLEFT  */
      case SDLK_RIGHT:  key = SDLK_RETURN;  break; /* vkRIGHT */
      case SDLK_j: 		key = SDLK_RETURN;  break; /* Action                 */
      case SDLK_k:      key = SDLK_ESCAPE;  break; /* Hold                   */
      case SDLK_u:      key = SDLK_UNKNOWN; break; /* Wheel clockwise        */
      case SDLK_i:      key = SDLK_UNKNOWN; break; /* Wheel counterclockwise */
      default: break;
    }
#endif
    return key;
...
if (event.key.keysym.scancode == 40 || event.key.keysym.scancode == 44)
{
SDL_Log("get 40 or 44\n");
	//endCommand();
        SDL_Event event;
        event.type = SDL_QUIT;
        SDL_PushEvent(&event);
